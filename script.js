...
let html = `<table> <thead> <tr> <th>License Plate</th> <th>Your Location</th> <th>Number of Miles</th> </tr> </thead> <tbody>`;

for (const [state, { location, miles }] of entries) {
const label = state.replace(/_/g, “ “).replace(/\b\w/g, c => c.toUpperCase());
html += `<tr> <td>${label}</td> <td>${location}</td> <td>${miles.toFixed(0)} <button class="removeBtn" data-state="${state}">Remove</button></td> </tr>`;
}
